...
std::pair<Position, Position> get_pair_pieces(position_indexes &indexes, positions &positions, std::default_random_engine rng)  {
    std::shuffle(indexes.begin(), indexes.end(), rng);
    //std::cout  << positions.size() << std::endl;
    //std::random_shuffle(indexes.begin(), indexes.end());
    return std::make_pair(positions[indexes[0]], positions[indexes[1]]);
}

positions initialize_corner_positions() {
    positions tmp_corner_positions;
...
positions initialize_edge_positions() {
    //done only once; dont care about performance
    positions tmp_corner_positions;
    for (uint_fast8_t i = 0; i < PUZZLE_SIZE; i++) {
        for (uint_fast8_t j = 0; j < PUZZLE_SIZE; j++) {
            if ((i == j) ||  (i == PUZZLE_SIZE -1 && j == 0) || (j == PUZZLE_SIZE - 1 && i == 0)) {
                continue;
            }
            if ((i != 0 && i != PUZZLE_SIZE - 1) && j != 0 && j != PUZZLE_SIZE - 1) {
                continue;
            }
            if ((j !=0 && j != (PUZZLE_SIZE - 1)) && (i !=0 && i != (PUZZLE_SIZE-1))) {
                continue;
            }
            Position tmp_position = {i, j};
            tmp_corner_positions.push_back(tmp_position);
        }
    }
    return tmp_corner_positions;
...
positions initialize_inner_positions() {
    ...
    for (uint_fast8_t i = 0; i < PUZZLE_SIZE; i++) {
        for (uint_fast8_t j = 0; j < PUZZLE_SIZE; j++) {
            ...
            Position tmp_position = {i, j};
            tmp_inner_positions.push_back(tmp_position);
        }
    }
    return tmp_inner_positions;
}

position_indexes get_range(int n) {
    position_indexes vect; 
    vect.reserve(n);
    for (int i = 0; i < n; i++) {
        vect.push_back(i);
    }
...
int count_correct_edges(board &board,  Piece ** rotated_pieces) {
    int num_correct_edges = 0;
    for (int i = 0; i < PUZZLE_SIZE; i++) {
        for (int j = 0; j < PUZZLE_SIZE; j++) {
            Piece *current_piece = &rotated_pieces[board[i * PUZZLE_SIZE + j].index][board[i * PUZZLE_SIZE + j].orientation];

            if(j != PUZZLE_SIZE - 1 &&
                    current_piece->right == rotated_pieces[board[i * PUZZLE_SIZE + j + 1].index][board[i * PUZZLE_SIZE + j + 1].orientation].left) {
                num_correct_edges += 1;
            }

            if (i != PUZZLE_SIZE - 1 &&
                    current_piece->bottom == rotated_pieces[board[(i + 1) * PUZZLE_SIZE + j].index][board[(i +1) * PUZZLE_SIZE + j].orientation].top) {
                num_correct_edges += 1;
            }
...
positions get_list_of_positions(int k, position_indexes &indexes, positions &_positions, std::default_random_engine rng) {
    ...
    while(tmp_positions.size() < k) {
        ...
        bool is_adjacent_to_other_positions = false;
        for(int j = 0; j < tmp_positions.size(); j++) {
            Position tmp_position = tmp_positions[j];
            if (tmp_position.i == potential_position.i && abs(tmp_position.j - potential_position.j) <= 1) { 
                is_adjacent_to_other_positions = true;
                break;
            }
            if (tmp_position.j == potential_position.j && abs(tmp_position.i - potential_position.i) <= 1) { 
                is_adjacent_to_other_positions = true;
                break;
            }
        }
        if (!is_adjacent_to_other_positions) {
            tmp_positions.push_back(potential_position);
        }
        i++;
    }
    return tmp_positions;
...
void print_list_of_positions (positions list_of_positions) {
    for (int i = 0; i < list_of_positions.size(); i++) {
        std::cout << "list of p: " <<  (int)list_of_positions[i].i << "-" << (int)list_of_positions[i].j << std::endl;
    }
}
...
int find_best_position_per_hole_tile (positions list_of_positions, board board, Piece ** rotated_pieces, int count_correct_edges) {
    ...
    for(int piece_index = 0; piece_index < n_matrix_rows; piece_index++) {
        for(int hole_index=0; hole_index < n_matrix_rows; hole_index++) {
            Position *current_position = &list_of_positions[hole_index]; 
            ...
            for (int orientation; orientation < 4; orientation++) {
                int n_correct_edges = 0;

                Piece *evaluated_piece = rotated_pieces[piece_index + orientation];
                if(evaluated_piece->top ==  rotated_pieces[board[(current_position->i - 1) * PUZZLE_SIZE + current_position->j].index][board[(current_position->i - 1) * PUZZLE_SIZE + current_position->j].orientation].bottom) {
                    n_correct_edges += 1;
                }
                if(evaluated_piece->bottom ==  rotated_pieces[board[(current_position->i + 1) * PUZZLE_SIZE + current_position->j].index][board[(current_position->i + 1) * PUZZLE_SIZE + current_position->j].orientation].top) {
                    n_correct_edges += 1;
                }
                if(evaluated_piece->left ==  rotated_pieces[board[(current_position->i) * PUZZLE_SIZE + current_position->j - 1].index][board[(current_position->i) * PUZZLE_SIZE + current_position->j + 1].orientation].right) {
                    n_correct_edges += 1;
                }
                if(evaluated_piece->right ==  rotated_pieces[board[(current_position->i) * PUZZLE_SIZE + current_position->j + 1].index][board[(current_position->i) * PUZZLE_SIZE + current_position->j + 1].orientation].left) {
                    n_correct_edges += 1;
                }
                if (n_correct_edges > max_correct_edges) {
                    max_correct_edges = n_correct_edges;
                    max_orientation = orientation;
                }
            }
            cost_matrix[piece_index].push_back(max_correct_edges);
