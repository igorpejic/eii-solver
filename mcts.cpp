...
std::tuple<state_t, int, bool>CustomMCTS::predict(int N=1000) {
    ...
    while (state.m_pieces.size()) {
        ...
        //cout << "Pieces size: " << state.m_pieces.size() << std::endl;
        for(int j = 0; j < state.m_pieces.size() * 4; j++) {
            ...
            m_pieces_placed += 1;
            //cout << piece[0] << piece[1] << piece[2] << piece[3] << ":" << state.m_pieces.size() <<  std::endl;
            ...
            if (!success) {
                ...
                //cout << "success" << endl;
                //cout << piece[0] << piece[1] << piece[2] << piece[3] << std::endl;
                ...
                for (int i = 0; i < state.m_pieces.size(); i++) {
                    std::array<int, 4> vec = {
                        state.m_pieces[i][0],
                        state.m_pieces[i][1],
                        state.m_pieces[i][2],
                        state.m_pieces[i][3]};
                    new_pieces.push_back(vec);
                }
...
std::tuple<int, std::vector<std::pair<std::array<int, 4>, std::array<int, 2>> >> CustomMCTS::perform_simulation(state_t state, std::array<int, 2> next_position) {
    ...
    std::vector<int> grid = state.m_grid;
